...
#include "internal/T1000EHardware.hpp"
...
{
    FLASH_APPLICATION_END_ADDR / T1000EHardware::getInstance().flash.PAGE_SIZE - 1, // CONTEXT_MODEM
    FLASH_APPLICATION_END_ADDR / T1000EHardware::getInstance().flash.PAGE_SIZE - 2, // CONTEXT_LR1MAC
    FLASH_APPLICATION_END_ADDR / T1000EHardware::getInstance().flash.PAGE_SIZE - 3, // CONTEXT_DEVNONCE
    FLASH_APPLICATION_END_ADDR / T1000EHardware::getInstance().flash.PAGE_SIZE - 4, // CONTEXT_SECURE_ELEMENT
};
...
{
    return T1000EHardware::getInstance().rtcTimer.getElapsedSeconds();
}
...
{
    return T1000EHardware::getInstance().rtcTimer.getElapsedMilliseconds();
}

uint32_t smtc_modem_hal_get_time_in_100us( void )
{
    return T1000EHardware::getInstance().rtcTimer.getElapsed100Microseconds();
}
...
void smtc_modem_hal_start_timer( const uint32_t milliseconds, void ( *callback )( void* context ), void* context )
{
    ...
    T1000EHardware::getInstance().timerOneshot.start(milliseconds, callback, context);
}

void smtc_modem_hal_stop_timer( void )
{
    T1000EHardware::getInstance().timerOneshot.stop();
}
...
{
    T1000EHardware& hardware = T1000EHardware::getInstance();
...
void smtc_modem_hal_context_restore( const modem_context_type_t ctx_type, uint8_t* buffer, const uint32_t size )
{
    ...
    if (size <= 0 || T1000EHardware::getInstance().flash.PAGE_SIZE < size) abort();

    const uint32_t page = FlashLbmConextPages[ctx_type];

    T1000EHardware::getInstance().flash.read(page, buffer, size);
}
...
void smtc_modem_hal_context_store( const modem_context_type_t ctx_type, const uint8_t* buffer, const uint32_t size )
{
    ...
    T1000EHardware::getInstance().flash.write(page, buffer, size);
}
...
{
    return T1000EHardware::getInstance().rng.read();
}
...
uint32_t smtc_modem_hal_get_random_nb_in_range( const uint32_t val_1, const uint32_t val_2 )
{
    ...
    {
        return T1000EHardware::getInstance().rng.read() % (maxValue - minValue + 1) + minValue;
    }
}
...
int32_t smtc_modem_hal_get_signed_random_nb_in_range( const int32_t val_1, const int32_t val_2 )
{
    ...
    {
        return T1000EHardware::getInstance().rng.read() + INT32_MIN;
    }
    else
    {
        return T1000EHardware::getInstance().rng.read() % (maxValue - minValue + 1) + minValue;
...
void smtc_modem_hal_irq_config_radio_irq( void ( *callback )( void* context ), void* context )
{
    ...
    T1000EHardware::getInstance().irqEvent.begin(RISING, callback, context);
}

void smtc_modem_hal_radio_irq_clear_pending( void )
{
    T1000EHardware::getInstance().irqEvent.clearIRQ();
}
