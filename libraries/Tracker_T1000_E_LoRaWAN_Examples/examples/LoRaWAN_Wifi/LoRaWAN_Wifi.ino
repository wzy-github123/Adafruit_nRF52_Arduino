...
static constexpr uint32_t WIFI_SCAN_PERIOD_DEFAULT = 60; // [sec.]
static constexpr uint32_t WIFI_SCAN_TIME = 5;            // [sec.]
...
static uint8_t wifi_scan_status = 0;
...
void MyLbmxEventHandlers::joined(const LbmxEvent &event)
{
    ...
    if (LbmxEngine::startAlarm(10) != SMTC_MODEM_RC_OK)
        abort();
}
...
{
    if (wifi_scan_status == 0) // wifi scan
    {
        if (LbmxEngine::startAlarm(WIFI_SCAN_TIME) != SMTC_MODEM_RC_OK)
            abort();
        ...
        if (wifi_mw_scan_start(0) != MW_RC_OK)
            abort();
    }
    else if (wifi_scan_status == 1) // send data
    {
        int32_t next_delay = WIFI_SCAN_PERIOD_DEFAULT - WIFI_SCAN_TIME;
        ...
        if (wifi_results.nbr_results)
        {
            ...
            for (uint8_t i = 0; i < wifi_results.nbr_results; i++)
            {
                ...
                memcpy(&app_data_buffer[app_data_len], wifi_results.results[i].mac_address, WIFI_AP_ADDRESS_SIZE);
                app_data_len += WIFI_AP_ADDRESS_SIZE;

                // Copy Access Point RSSI address in result buffer
                app_data_buffer[app_data_len] = wifi_results.results[i].rssi;
                app_data_len += WIFI_AP_RSSI_SIZE;
            }

            Serial.println("Sending uplink containing found wifi networks");
...
{
    delay(1000);
    Serial.println("\n###### ===== T1000-E WIFI scan example ==== ######\n");

    // Start lora basics modem
    lbmT1000E.begin();
    ...
    if (wifi_mw_init(lbmT1000E.getRadio(), 0) != MW_RC_OK)
        abort();
